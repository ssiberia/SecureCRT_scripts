...
def Main():
	...
	customer = []
	for line in lines:
		if line.find('report') != -1: # csak hogy véletlenül se!
			continue
		if line.find('ustomer') != -1:
			if line.find('admin down') != -1:
				customer.append(str(line.split(' ')[0]))
	TAB.Screen.Send("conf te" + chr(13))
	for i in customer:
		i = i.lstrip()
		if i.find("Vl") != -1:
			TAB.Screen.Send("no spanning-tree Vlan " + str(i[2:]) + chr(13))
	for i in customer:
		i = i.lstrip()
		TAB.Screen.Send("int " + str(i) + chr(13))
		TAB.Screen.Send("no shut" + chr(13))
